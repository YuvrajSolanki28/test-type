    ...
    <div className="min-h-screen bg-linear-to-br from-[#0a0a0f] via-[#0f0f1a] to-[#1a0f1f] text-white">
      <div className="max-w-7xl mx-auto px-6 py-24">
        ...
        <div className="grid md:grid-cols-4 gap-6 mb-12">
          {[
            { icon: Zap, label: "Total Tests", value: stats.totalTests, color: "from-blue-500 to-cyan-500" },
            { icon: TrendingUp, label: "Average WPM", value: stats.averageWpm, color: "from-purple-500 to-pink-500" },
            { icon: Target, label: "Best WPM", value: stats.bestWpm, color: "from-cyan-500 to-blue-500" },
            { icon: Clock, label: "Total Time", value: `${Math.round(stats.totalTime / 60)}m`, color: "from-pink-500 to-red-500" },
          ].map((stat, index) => (
            <motion.div
              key={stat.label}
        ...
        {wpmData.length > 0 && (
          ...
          >
            <h2 className="text-2xl font-bold mb-6">WPM Progress (Last 20 Tests)</h2>
              ...
              {wpmData.map((data, index) => (
                <div key={index} className="flex-1 flex flex-col items-center gap-2">
                  <div className="w-full relative group">
                    <motion.div
                      initial={{ height: 0 }}
                      animate={{ height: `${(data.wpm / maxWpm) * 100}%` }}
                      transition={{ delay: index * 0.05, duration: 0.5 }}
                      className="w-full bg-linear-to-t from-blue-500 to-purple-600 rounded-t-lg min-h-1 hover:from-blue-400 hover:to-purple-500 transition-colors cursor-pointer"
                    />
          ...
          {recentTests.length === 0 ? (
            ...
            <div className="space-y-3">
              {recentTests.map((test) => (
                <div
                  key={test.id}
                  className="backdrop-blur-xl bg-white/5 border border-white/10 rounded-xl p-4 flex items-center justify-between hover:bg-white/10 transition-colors"
                >
                  ...
                  <div className="flex items-center gap-6">
                    <div className="text-right">
                      <div className="text-2xl font-bold text-blue-400">{test.wpm}</div>
                      <div className="text-xs text-white/50">WPM</div>
                    </div>
                    <div className="text-right">
                      <div className="text-2xl font-bold text-purple-400">{test.accuracy}%</div>
                      <div className="text-xs text-white/50">Accuracy</div>
                    </div>
                    <div className="text-right">
                      <div className="text-2xl font-bold text-cyan-400">{test.time}s</div>
