...
export function Leaderboard() {
  ...
  if (loading) {
    return (
      <Loading variant="fullscreen" text="Loading leaderboard..." />
    );
  }
            ...
            >
              <div className="flex items-center gap-3 mb-6">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${
                  board.difficulty === 'easy' ? 'bg-green-500/20' : 
                  board.difficulty === 'medium' ? 'bg-blue-500/20' : 'bg-red-500/20'
                }`}>
                  <Trophy className={`w-5 h-5 ${
                    board.difficulty === 'easy' ? 'text-green-400' : 
                    board.difficulty === 'medium' ? 'text-blue-400' : 'text-red-400'
                  }`} />
                </div>
                ...
                <div className="space-y-2">
                  {board.scores.map((score, index) => (
                    <motion.div
                      ...
                      className={`backdrop-blur-xl bg-white/5 border border-white/10 rounded-xl p-4 flex items-center justify-between ${
