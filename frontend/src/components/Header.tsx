...
export function Header({
  ...
  const buttonClassName = (isActive: boolean) => `
    ...
    ${isActive 
      ...
      : 'backdrop-blur-md bg-white/5 border border-white/10 text-white/70 hover:text-white hover:bg-white/15 hover:border-white/30'
    }
    ${disabled ? 'opacity-50 cursor-not-allowed hover:scale-100' : ''}
  `;

  const selectClassName = "backdrop-blur-md bg-white/5 border border-white/10 rounded-xl px-4 py-2 text-white disabled:opacity-50 transition-all duration-200 hover:border-white/40 focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-400/50";
  ...
  return (
    <motion.header
      ...
      className="fixed top-0 left-0 right-0 z-40 backdrop-blur-xl bg-white/5 border-b border-white/10 shadow-xl shadow-black/20"
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 sm:py-4">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
          ...
          >
            <div className="p-2 bg-gradient-to-r from-indigo-500 to-purple-600 rounded-xl shadow-lg shadow-indigo-500/50">
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3">
            {/* Text Type Buttons */}
            <motion.div 
              className="backdrop-blur-md bg-white/5 border border-white/10 rounded-2xl p-1"
